...
const SOCKET_CONFIGS = [
  ...
  {
    provider: 'Binance BookTicker',
    endpoint: 'wss://stream.binance.com:9443/ws/btcusdt@bookTicker',
    color: '#f0b90b',
    parsePrice: (msg: any) => parseFloat(msg.b), // best bid
    parseEventTime: (msg: any) => msg.E ?? null,
  },
  {
    provider: 'Binance AggTrade',
  ...
  {
    provider: 'Binance Depth100ms',
    endpoint: 'wss://stream.binance.com:9443/ws/btcusdt@depth@100ms',
    color: '#f0b90b',
    parsePrice: (msg: any) => msg.b?.[0]?.[0] ? parseFloat(msg.b[0][0]) : null, // best bid from depth
    parseEventTime: (msg: any) => msg.E ?? null,
  },
  {
    provider: 'Binance Futures Trade',
  ...
  {
    provider: 'Binance Futures BookTicker',
...
export default function SocketsTab() {
  ...
  const updateResult = useCallback((provider: string, patch: Partial<SocketResult>) => {
    setResults(prev => prev.map(r => r.provider === provider ? { ...r, ...patch } : r));
  }, []);
  ...
  const connectOne = useCallback((index: number) => {
    ...
    // Close existing
    const existing = wsRefs.current.get(provider);
    if (existing) existing.close();

    updateResult(provider, { status: 'connecting', errorMsg: null, messageCount: 0, lastPrice: null, latencyMs: null, avgLatencyMs: null, minLatencyMs: null, maxLatencyMs: null, messagesPerSecond: 0 });

    latencySamples.current.set(provider, []);
    msgCountRefs.current.set(provider, { count: 0, startTime: Date.now() });

    try {
      const ws = new WebSocket(cfg.endpoint);
      wsRefs.current.set(provider, ws);

      ws.onopen = () => {
        updateResult(provider, { status: 'connected' });
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (price !== null && !isNaN(price) && price > 0) {
            ...
            const counter = msgCountRefs.current.get(provider);
            if (counter) {
              counter.count++;
              const elapsed = (now - counter.startTime) / 1000;
              const mps = elapsed > 0 ? counter.count / elapsed : 0;
              updateResult(provider, { messagesPerSecond: Math.round(mps * 10) / 10 });
            }

            // Track latency samples
            if (latency !== null && latency >= 0 && latency < 60000) {
              const samples = latencySamples.current.get(provider) ?? [];
              samples.push({ latency, time: now });
              // Keep last 100 samples
              if (samples.length > 100) samples.shift();
              latencySamples.current.set(provider, samples);
      ...
      ws.onerror = () => {
        updateResult(provider, { status: 'error', errorMsg: 'Connection error' });
      };

      ws.onclose = (event) => {
        const wasConnected = results.find(r => r.provider === provider)?.status === 'connected';
        if (wasConnected) {
          updateResult(provider, { status: 'error', errorMsg: `Closed (code ${event.code})` });
        } else {
          updateResult(provider, { status: 'error', errorMsg: `Failed to connect (code ${event.code})` });
        }
      };
    } catch (e: any) {
      updateResult(provider, { status: 'error', errorMsg: e.message });
    }
  ...
  const disconnectAll = useCallback(() => {
    wsRefs.current.forEach((ws, provider) => {
      ws.close();
      updateResult(provider, { status: 'disconnected' });
    });
  ...
  const disconnectSingle = useCallback((provider: string) => {
    const ws = wsRefs.current.get(provider);
    if (ws) {
      ws.close();
      wsRefs.current.delete(provider);
    }
    updateResult(provider, { status: 'disconnected' });
  }, [updateResult]);
  ...
  const bestProvider = ranked.length > 0 ? ranked[0].provider : null;
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {ranked.length > 0 && (
        <div className="bg-gray-900 rounded-xl p-5">
          ...
          <div className="flex gap-4 flex-wrap">
            {ranked.map((r, i) => (
              <div key={r.provider} className={`flex items-center gap-2 px-3 py-2 rounded-lg ${i === 0 ? 'bg-green-900/30 border border-green-700' : 'bg-gray-800'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {SOCKET_CONFIGS.map((cfg, i) => {
          const r = results.find(res => res.provider === cfg.provider)!;
          const isBest = r.provider === bestProvider;
          ...
          return (
            <div key={cfg.provider} className={`bg-gray-900 rounded-xl p-5 space-y-3 border ${isBest ? 'border-green-700' : 'border-gray-800'}`}>
              {/* Card Header */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-2.5 h-2.5 rounded-full ${statusColor}`} />
                  <div>
                    <h3 className="text-sm font-bold text-white">{cfg.provider}</h3>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {r.status === 'connected' ? (
                    <button onClick={() => disconnectSingle(r.provider)} className="text-xs text-red-400 hover:text-red-300 px-2 py-1 rounded bg-red-900/20">Stop</button>
                  ) : (
