...
import { fiat, usd, toFiat } from '../engine/formatters';
    ...
    <div className="p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-gray-900 rounded-xl p-5 text-center">
          ...
          <p className="text-xl font-bold text-green-400">{fiat(toFiat(pt.demoBalance))}</p>
        </div>
        <div className="bg-gray-900 rounded-xl p-5 text-center">
          ...
          <p className={`text-xl font-bold ${pt.totalProfit >= 0 ? 'text-green-400' : 'text-pink-400'}`}>
            {pt.totalProfit >= 0 ? '+' : ''}{fiat(toFiat(pt.totalProfit))}
          </p>
        </div>
        <div className="bg-gray-900 rounded-xl p-5 text-center">
          ...
          <div className="grid grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className={`text-base font-bold ${pt.unrealizedProfit(vm.activeSignal.price) >= 0 ? 'text-green-400' : 'text-pink-400'}`}>
                {pt.unrealizedProfit(vm.activeSignal.price) >= 0 ? '+' : ''}{fiat(toFiat(pt.unrealizedProfit(vm.activeSignal.price)))}
              </p>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {pt.history.map((trade: any, i: number) => (
                <tr key={trade.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  ...
                  <td className="py-2 px-2 text-right text-gray-300">{usd(trade.entryPrice)}</td>
                  <td className="py-2 px-2 text-right text-gray-300">{usd(trade.exitPrice)}</td>
                  <td className="py-2 px-2 text-right text-gray-300">{fiat(toFiat(trade.investedAmount))}</td>
                  <td className={`py-2 px-2 text-right font-bold ${trade.profit >= 0 ? 'text-green-400' : 'text-pink-400'}`}>
                    {trade.profit >= 0 ? '+' : ''}{fiat(toFiat(trade.profit))}
                  </td>
