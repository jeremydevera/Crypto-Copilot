      ...
      <div className="mx-auto max-w-5xl p-4 sm:p-6 space-y-4">
            ...
            {displayCandle && (
              <>
                <span className={`font-mono text-2xl font-semibold ${stats?.color ?? 'text-white'}`}>
                  {usd(displayCandle.close)}
                </span>
                <span className={`font-mono text-sm ${stats?.color ?? 'text-gray-400'}`}>
                  {stats ? `${stats.change >= 0 ? '+' : ''}${usd(stats.change)} (${stats.changePercent.toFixed(2)}%)` : ''}
                </span>
              </>
            )}
          </div>
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
            <Stat label="Open" value={displayCandle ? usd(displayCandle.open) : '--'} />
            <Stat label="High" value={displayCandle ? usd(displayCandle.high) : '--'} />
            <Stat label="Low" value={displayCandle ? usd(displayCandle.low) : '--'} />
            <Stat label="Close" value={displayCandle ? usd(displayCandle.close) : '--'} />
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <IndicatorCard label="EMA 9" value={sig.fiveMinute.ema9 !== null ? usd(sig.fiveMinute.ema9) : '--'} color="text-[#f0b90b]" />
          <IndicatorCard label="EMA 21" value={sig.fiveMinute.ema21 !== null ? usd(sig.fiveMinute.ema21) : '--'} color="text-blue-400" />
          <IndicatorCard label="EMA 50" value={sig.fiveMinute.ema50 !== null ? usd(sig.fiveMinute.ema50) : '--'} color="text-purple-400" />
