...
import { fiat, usd, percent, number, toFiat, getExchangeRate } from '../engine/formatters';
...
export default function CalculatorTab({ vm }: CalculatorTabProps) {
  ...
  const [customEntry, setCustomEntry] = useState(sig.price > 0 ? sig.price.toFixed(2) : '');
  ...
  // Update entry price when crypto price changes
  useEffect(() => {
    if (sig.price > 0) {
      setCustomEntry(sig.price.toFixed(2));
    }
  }, [sig.price]);
  ...
  // Structure levels are in USD (crypto prices)
  const structStopLoss = sig.stopLoss || 0;
  const structTarget1 = sig.target1 || 0;
  const structTarget2 = sig.target2 || 0;
  ...
  if (useStructureLevels && sig.price > 0) {
    // Use structure-based levels from the signal (in USD)
    stopLoss = structStopLoss || adjustedEntry * (1 - stopPct / 100);
    target1 = structTarget1 || adjustedEntry * (1 + t1Pct / 100);
    target2 = structTarget2 || adjustedEntry * (1 + t2Pct / 100);
  } else {
  ...
  // Convert investment from fiat to USD for calculations
  const usdInvestment = vm.fiatCurrency === 'USD' ? investment : investment / getExchangeRate();
  const quantity = usdInvestment > 0 && adjustedEntry > 0 ? usdInvestment / adjustedEntry : 0;
  const buyFee = usdInvestment * (feePct / 100);
  const grossSellValue = quantity * target2;
  const sellFee = grossSellValue * (feePct / 100);
  const netProfitUsd = grossSellValue - sellFee - usdInvestment - buyFee;
  const profitPercent = usdInvestment > 0 ? (netProfitUsd / usdInvestment) * 100 : 0;
  const lossAtStopUsd = usdInvestment * (risk / adjustedEntry);
  const lossPercent = adjustedEntry > 0 ? (lossAtStopUsd / usdInvestment) * 100 : 0;

  // Position sizing
  const usdBalance = vm.fiatCurrency === 'USD' ? pt.demoBalance : pt.demoBalance; // demoBalance is always USD
  const accountRisk = usdBalance * 0.01; // 1% risk
  ...
  const unrealizedProfit = hasPosition ? toFiat(pt.unrealizedProfit(sig.price)) : 0;
  const cryptoPair = vm.cryptoPair.split('/')[0];
    ...
    <div className="p-6 space-y-5 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-5">
        <div className="bg-gray-900 rounded-xl p-5 space-y-4">
          ...
          <div>
            <label className="text-sm text-gray-400 block mb-1.5">Entry Price (USD)</label>
            <input
              type="number"
              value={customEntry}
          ...
          <div>
            <label className="text-sm text-gray-400 block mb-1.5">Investment Amount ({vm.fiatCurrency})</label>
            <input
              type="number"
              ...
              className="w-full bg-gray-800 rounded-lg px-4 py-2.5 text-sm text-gray-200 border border-gray-700 focus:border-green-500 outline-none transition-colors"
            />
            {investment > 0 && adjustedEntry > 0 && (
              <p className="text-xs text-gray-500 mt-1">
                ≈ {number(quantity, 6)} {cryptoPair}
              </p>
            )}
          </div>
        ...
        <div className="bg-gray-900 rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-2.5">
            ...
            <CalcRow label={`Risk per ${cryptoPair}`} value={usd(risk)} />
            <div className="border-t border-gray-800 my-2" />
            <CalcRow label="Buy Fee" value={fiat(toFiat(buyFee))} color="text-gray-400" />
            <CalcRow label="Sell Fee (at T2)" value={fiat(toFiat(sellFee))} color="text-gray-400" />
            <CalcRow label="Net Profit (at T2)" value={fiat(toFiat(netProfitUsd))} color={netProfitUsd >= 0 ? 'text-green-400' : 'text-pink-400'} />
            <CalcRow label="Profit %" value={percent(profitPercent)} color={profitPercent >= 0 ? 'text-green-400' : 'text-pink-400'} />
            <CalcRow label="Max Loss (at SL)" value={fiat(toFiat(lossAtStopUsd))} color="text-pink-400" />
      ...
      <div className="bg-gray-900 rounded-xl p-5 space-y-3">
        ...
        <div className="grid grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-base font-medium text-white">{fiat(toFiat(pt.demoBalance))}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500 mb-1">Account Risk (1%)</p>
            <p className="text-base font-medium text-yellow-400">{fiat(toFiat(accountRisk))}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500 mb-1">Suggested Position</p>
            <p className="text-base font-medium text-green-400">{number(suggestedSize, 6)} {cryptoPair}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500 mb-1">Position Value</p>
            <p className="text-base font-medium text-green-400">{fiat(toFiat(suggestedValue))}</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-gray-900 rounded-xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-base font-medium text-white">{fiat(toFiat(pt.openPosition.investedAmount))}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Quantity</p>
              <p className="text-base font-medium text-white">{pt.openPosition.remainingQuantity.toFixed(6)} {cryptoPair}</p>
            </div>
            <div>
              ...
              <p className={`text-base font-bold ${unrealizedProfit >= 0 ? 'text-green-400' : 'text-pink-400'}`}>
                {unrealizedProfit >= 0 ? '+' : ''}{fiat(unrealizedProfit)}
              </p>
            </div>
          </div>
        </div>
