...
import { syncOpenPositionToSupabase, syncClosedTradeToSupabase, deleteClosedTradeFromSupabase } from '../services/SupabaseSync';
...
export class PaperTradingStore {
  ...
  buy(symbol: string, price: number, amount: number): string | null {
    ...
    this.saveToDisk();
    syncOpenPositionToSupabase(this.openPosition).catch(() => {});
    return null;
  }
  ...
  sell(price: number): { trade: ClosedPaperTrade } | { error: string } {
    ...
    this.saveToDisk();
    syncClosedTradeToSupabase(closedTrade).catch(() => {});
    syncOpenPositionToSupabase(null).catch(() => {});
  ...
  sellPartial(price: number, percent: number): { trade: ClosedPaperTrade | null } | { error: string } {
    ...
    if (updated.remainingQuantity * slippedPrice < 1.0) {
      ...
      syncOpenPositionToSupabase(updated).catch(() => {});
    }
    this.saveToDisk();
    return { trade: closedTrade };
  }
  ...
  deleteTrade(index: number) {
    const trade = this.history[index];
    this.history.splice(index, 1);
    this.saveToDisk();
    if (trade) deleteClosedTradeFromSupabase(trade.id).catch(() => {});
  }
